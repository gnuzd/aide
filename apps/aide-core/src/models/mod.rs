...
impl ModelRegistry {
    pub fn new() -> Self {
        ...
        if !models_dir.exists() {
            fs::create_dir_all(&models_dir).expect("Could not create models directory");
        }
        ...
        if let Ok(loaded_models) = registry.load_registry_file() {
            registry.models = loaded_models;
        } else {
            // Fallback to defaults if no registry file exists
            registry.models = Self::default_models();
        }
    ...
    fn load_registry_file(&self) -> anyhow::Result<Vec<Model>> {
        let path = self.get_registry_path();
        if !path.exists() {
            return Err(anyhow::anyhow!("Registry file not found"));
        }
    ...
    pub fn save_registry_file(&self) -> anyhow::Result<()> {
        let path = self.get_registry_path();
    ...
    pub async fn sync_models(&mut self) -> anyhow::Result<()> {
        // In a real app, this would be a real URL. 
        // For now, we simulate by ensuring we have a local registry or updating from defaults.
        // We'll also provide a way to fetch from a remote JSON if specified.
        const REGISTRY_URL: &str = "https://raw.githubusercontent.com/google-gemini/aide/main/registry.json";
        
        println!("Syncing model registry...");
        let client = Client::new();
        match client.get(REGISTRY_URL).timeout(std::time::Duration::from_secs(5)).send().await {
            Ok(res) => {
                if res.status().is_success() {
                    let remote_models: Vec<Model> = res.json().await?;
                    self.models = remote_models;
                    self.save_registry_file()?;
                    println!("Registry updated from remote.");
                }
            }
            Err(_) => {
                println!("Could not reach remote registry. Using local cache.");
            }
        }
        Ok(())
    }
    ...
    pub fn get_compatible_models(&self, specs: &SystemSpecs, model_type: ModelType) -> Vec<&Model> {
        let mut compatible: Vec<&Model> = self.models
            .iter()
            .filter(|m| m.model_type == model_type && m.min_ram_gb <= specs.total_memory_gb)
            .collect();
            
        // Sort: models that fit comfortably in RAM first, then by quality
        compatible.sort_by(|a, b| {
            let ram_cmp = a.min_ram_gb.cmp(&b.min_ram_gb);
            if ram_cmp == std::cmp::Ordering::Equal {
